...
#grep -v '#' withoutcolumns.txt > yyjdata.txt &&
...
#starting stream
sed '1,/super_stream_server|Scene switched to  Live/d' readydata.txt > aaaaa.txt && 
#search all lines that contains # and delete them
awk '!/just earned/ &&  !/sending messages too quickly/ && !/emote-only/ && !/You can find your currently available/ && !/raiders from/ && !/redeemed/ && !/streamelements/ && !/innytty is live!/' aaaaa.txt > yyj.txt
